...
import com.github.nexmark.flink.generator.model.LongGenerator;
...
/** AuctionGenerator. */
public class AuctionGeneratorImproved {
  /**
  ...
  public static Auction nextAuction(
    ...
    if (random.nextInt(config.getHotSellersRatio()) > 0) {
      ...
      seller = (PersonGeneratorImproved.lastBase0PersonId(config, eventId) / HOT_SELLER_RATIO) * HOT_SELLER_RATIO;
    } else {
      seller = PersonGeneratorImproved.nextBase0PersonId(eventId, random, config);
    }
