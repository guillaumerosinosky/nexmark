...
import com.github.nexmark.flink.generator.model.PriceGenerator;
import com.github.nexmark.flink.generator.model.StringsGenerator;
...
/** Generates bids. */
public class BidGeneratorImproved {
  ...
  public static Bid nextBid(long eventId, Random random, long timestamp, GeneratorConfig config) {
    ...
    if (random.nextInt(config.getHotAuctionRatio()) > 0) {
      ...
      auction = (AuctionGeneratorImproved.lastBase0AuctionId(config, eventId) / HOT_AUCTION_RATIO) * HOT_AUCTION_RATIO;
    } else {
      auction = AuctionGeneratorImproved.nextBase0AuctionId(eventId, random, config);
    }
    ...
    if (random.nextInt(config.getHotBiddersRatio()) > 0) {
      ...
      bidder = (PersonGeneratorImproved.lastBase0PersonId(config, eventId) / HOT_BIDDER_RATIO) * HOT_BIDDER_RATIO + 1;
    } else {
      bidder = PersonGeneratorImproved.nextBase0PersonId(eventId, random, config);
    }
